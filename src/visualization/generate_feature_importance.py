...
class SimpleDataLoader:
    ...
    def load_data(self):
        ...
        # Note: The .npz has raw embeddings. The names are generic unless we map them, 
        # but for GRU output (hidden state), features are just dimensions 0..127
    ...
    def _load_npz(self, path, target_ids, prefix):
        ...
        if 'node_names' in data:
            ...
            if len(raw_names) == dim:
                ...
                logger.warning(f"Feature name mismatch in {path}: {len(raw_names)} names for {dim} cols. Using generic.")
    ...
    def _load_struct(self, target_ids):
        # We assume structured embeddings are present
...
def main():
    ...
    # Covert feature names to list for mapping
    model.get_booster().feature_names = list(feat_names)
    ...
    logger.info("Saved feature_importance_gain.png and feature_importance_weight.png")
